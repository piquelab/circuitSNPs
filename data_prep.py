...
def make_dsQTL_frames_for_replicates_5():
    
    beers_df = pd.read_excel('/nfs/rprscratch/deep_learning/beer_data/ng.3331-S2.xlsx')
    beers_df = beers_df[['SNPname2','gkm_SVM','abs_gkm_SVM','label']]
    X=pd.read_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/compendium_circuitSNPs_with_footsnps_thresh_5.h5')
    dsQTL_X = beers_df.merge(X,right_on='rsID',left_on='SNPname2',how='left')

    foot=pd.read_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/centiSNPs_footprints.h5')
    foot['rsID'] = X.rsID


    dsQTL_foot = beers_df.merge(foot,right_on='rsID',left_on='SNPname2',how='left')

    dsQTL_X.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/replicates/dsQTL_df_thresh_5.h5','df',mode='w')

    dsQTL_foot.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/replicates/dsQTL_df_foot_thresh_5.h5','df',mode='w')

def make_dsQTL_frames_for_replicates_3():
    
    beers_df = pd.read_excel('/nfs/rprscratch/deep_learning/beer_data/ng.3331-S2.xlsx')
    beers_df = beers_df[['SNPname2','gkm_SVM','abs_gkm_SVM','label']]
    X=pd.read_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/compendium_circuitSNPs_with_footsnps.h5')
    dsQTL_X = beers_df.merge(X,right_on='rsID',left_on='SNPname2',how='left')
    ...
    dsQTL_X.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/replicates/dsQTL_df_thresh_3.h5','df',mode='w')

    dsQTL_foot.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/replicates/dsQTL_df_foot_thresh_3.h5','df',mode='w')
...
def make_all_circuitSNPsD_including_non_effect(thresh = 3):
    all_snps_bed_file = '/wsu/home/groups/piquelab/allCentipede/updatedModel/pwmRescan/fullAllgenome/results/all.centiSnps.unique.bed.gz'
    centiSNPs_df = pd.read_csv(all_snps_bed_file,delim_whitespace=True,header=None,names=["chr","start","stop"])
    centiSNPs_df.set_index(['chr', 'stop'], inplace=True)
    ...
    for idx,footprint_snp in enumerate(footprint_snps):
        ...
        binding_up = (footsnp_df['ref_lr']>=footsnp_df['alt_lr'])& (np.abs(footsnp_df['ref_lr']-footsnp_df['alt_lr'])>=thresh)

        binding_down = (footsnp_df['ref_lr']<footsnp_df['alt_lr'])& (np.abs(footsnp_df['ref_lr']-footsnp_df['alt_lr'])>=thresh)

        footsnp_df['direct_snps'] = 1
        centiSNPs_df[motif_name] = 0

        merge_idx = footsnp_df.index.intersection(centiSNPs_df.index)

        footsnp_df.loc[binding_up,'direct_snps'] = 2
        footsnp_df.loc[binding_down,'direct_snps'] = 3
    ...
    colnames[1]='start'
    centiSNPs_df = centiSNPs_df[colnames]
    centiSNPs_df.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/compendium_circuitSNPs_with_footsnps_thresh_{0}.h5'.format(thresh),'df',mode='w')


def make_all_circuitSNPsD_lcl_for_supp():
    all_snps_bed_file = '/wsu/home/groups/piquelab/allCentipede/updatedModel/pwmRescan/fullAllgenome/results/all.centiSnps.unique.bed.gz'
    centiSNPs_df = pd.read_csv(all_snps_bed_file,delim_whitespace=True,header=None,names=["chr","start","stop"])
    centiSNPs_df.set_index(['chr', 'stop'], inplace=True)
    ...
    colnames[1]='start'
    centiSNPs_df = centiSNPs_df[colnames]
    centiSNPs_df.to_hdf('/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/compendium_circuitSNPs_lcl_supp.h5','df',mode='w')
...
def make_centiSNPs_footprints_windowed_lcl(flank_size=150):
    all_snps_bed_file = "/wsu/home/al/al37/al3786/CENNTIPEDE/circuitSNPs/compendium_predictions/old/compendium_effect_snps.bed"
